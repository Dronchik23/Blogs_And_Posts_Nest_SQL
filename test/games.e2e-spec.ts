...
describe('pair-game-quiz/pairs tests (e2e)', () => {
  ...
  async function createGameInPending(firstPlayerToken: string) {
    ...
    const gameInPending = createResponseForGame.body;

    expect(gameInPending.id).toBeDefined();
    expect(gameInPending.status).toEqual(GameStatuses.PendingSecondPlayer);

    return gameInPending;
  }
  async function createGame(
    ...
    expect(getResponseForQuestions.body.items.length).not.toBe(0);
    ...
    const gameInPending = createResponseForPairUser1.body;

    expect(gameInPending.id).toBeDefined();
    expect(gameInPending.status).toEqual(GameStatuses.PendingSecondPlayer);
    expect(gameInPending.questions).toEqual(gameInPending.questions);
    expect(gameInPending.pairCreatedDate).toEqual(
      gameInPending.pairCreatedDate,
    );
    ...
    const game: GameViewModel = createResponseForPairUser2.body;

    expect(game.id).toBeDefined();
    expect(game.status).toEqual(GameStatuses.Active);
  ...
  async function callFinishedGame(
    firstPlayerToken: string,
    secondPlayerToken: string,
  ) {
    for (let i = 0; i < 4; i++) {
    ...
    expect(finishedGame.firstPlayerProgress.player.login).toEqual(user1.login);
    ...
    expect(finishedGame.secondPlayerProgress.player.login).toEqual(user2.login);
  ...
  describe('pair-game-quiz/pairs', () => {
    describe('get game by gameId tests', () => {
      beforeEach(async () => {
        ...
        game = await createGameInPending(accessToken1);
        console.log('game', game);
      });
    ...
    describe('create games games for two players tests', () => {
      ...
      it('should create games for two players with correct input data', async () => {
        ...
        const fullGame: GameViewModel = createResponseForPairUser2.body;

        expect(fullGame.id).toBeDefined();
        expect(fullGame.status).toEqual(GameStatuses.Active);
        expect(fullGame.firstPlayerProgress.player.id).toEqual(user1.id);
        expect(fullGame.firstPlayerProgress.player.login).toEqual(user1.login);
        expect(fullGame.firstPlayerProgress.answers).toEqual([]);
        expect(fullGame.secondPlayerProgress.player.id).toEqual(user2.id);
        expect(fullGame.secondPlayerProgress.player.login).toEqual(user2.login);
        expect(fullGame.secondPlayerProgress.answers).toStrictEqual([]);
        expect(fullGame.questions).toBeDefined();
        expect(fullGame.pairCreatedDate).toBeDefined();
        expect(fullGame.startGameDate).toBeDefined();
        expect(fullGame.finishGameDate).toBeNull();
      });
      it('Should return 403 if current user1 is already in active game', async () => {
        ...
        const fullGame: GameViewModel = responseForGameUser2.body;
    ...
    describe('send answer tests', () => {
      beforeEach(async () => {
        ...
        game = await createGame(accessToken1, accessToken2);
      });
      ...
      it('first player must get additional score for finished first', async () => {
        ...
        const finishedGame: GameViewModel = responseForFinishedGame.body;
        console.log('finishedGame', finishedGame.firstPlayerProgress.answers);
    ...
    describe('dependent tests', () => {
      it('first', async () => {
        ...
        expect(currentGame.secondPlayerProgress.answers.length).toBe(0);
        expect(currentGame.secondPlayerProgress.answers).toEqual([]);
        expect(currentGame.questions.length).toBe(5);
        expect(currentGame.pairCreatedDate).toBeDefined();
        expect(currentGame.startGameDate).toBeDefined();
        expect(currentGame.finishGameDate).toBeNull();
        ...
        expect(currentGame2.firstPlayerProgress.answers).toBeDefined();
        expect(
          currentGame2.firstPlayerProgress.answers[0].answerStatus,
        ).toEqual(AnswerStatuses.Correct);
        expect(currentGame2.firstPlayerProgress.score).toBeDefined();
        ...
        expect(currentGame2.secondPlayerProgress.answers.length).toBe(1);
        expect(
          currentGame2.secondPlayerProgress.answers[0].answerStatus,
        ).toEqual(AnswerStatuses.Incorrect);
        expect(currentGame2.questions.length).toBe(5);
        ...
        const responseForCurrentGame3 = await request(server)
          .get(currentGameUrl)
          .set('Authorization', `Bearer ${accessToken2}`)
          .expect(200);

        const currentGame3: GameViewModel = responseForCurrentGame3.body;

        expect(currentGame3.id).toBeDefined();
        expect(currentGame3.status).toEqual(GameStatuses.Active);
        expect(currentGame3.firstPlayerProgress.player.id).toEqual(user1.id);
        expect(currentGame3.firstPlayerProgress.player.login).toEqual(
          user1.login,
        );
        expect(currentGame3.firstPlayerProgress.answers).toBeDefined();
        expect(
          currentGame3.firstPlayerProgress.answers[0].answerStatus,
        ).toEqual(AnswerStatuses.Correct);
        expect(currentGame3.firstPlayerProgress.score).toBeDefined();
        expect(currentGame3.secondPlayerProgress.player.id).toEqual(user2.id);
        expect(currentGame3.secondPlayerProgress.player.login).toEqual(
          user2.login,
        );
        expect(currentGame3.secondPlayerProgress.answers.length).toBe(2);
        expect(
          currentGame3.secondPlayerProgress.answers[0].answerStatus,
        ).toEqual(AnswerStatuses.Incorrect);
        expect(
          currentGame3.secondPlayerProgress.answers[1].answerStatus,
        ).toEqual(AnswerStatuses.Correct);
        expect(currentGame3.questions.length).toBe(5);
        expect(currentGame3.pairCreatedDate).toBeDefined();
        expect(currentGame3.startGameDate).toBeDefined();
        expect(currentGame3.finishGameDate).toBeNull();
      });
      it('second', async () => {
        const user3PlusToken = await createUser(3);
        const user3 = user3PlusToken[0];
        ...
        const user4PlusToken = await createUser(4);
        const user4 = user4PlusToken[0];
        expect(user4).toBeDefined();
        const accessToken4 = user4PlusToken[1];
        expect(accessToken4).toBeDefined();

        await createGame(accessToken3, accessToken4);
        ...
        expect(currentGame.id).toBeDefined();
        expect(currentGame.status).toEqual(GameStatuses.Active);
        expect(currentGame.firstPlayerProgress.player.id).toEqual(user3.id);
        expect(currentGame.firstPlayerProgress.player.login).toEqual(
          user3.login,
        );
        expect(currentGame.firstPlayerProgress.answers[0].answerStatus).toEqual(
        ...
        expect(currentGame.firstPlayerProgress.score).toBeDefined();
        expect(currentGame.secondPlayerProgress.player.id).toEqual(user4.id);
        expect(currentGame.secondPlayerProgress.player.login).toEqual(
          user4.login,
        );
        expect(currentGame.secondPlayerProgress.answers.length).toBe(0);
        ...
        expect(currentGame2.id).toBeDefined();
        expect(currentGame2.status).toEqual(GameStatuses.Active);
        expect(currentGame2.firstPlayerProgress.player.id).toEqual(user3.id);
        expect(currentGame2.firstPlayerProgress.player.login).toEqual(
          user3.login,
        );
        expect(currentGame2.firstPlayerProgress.answers).toBeDefined();
        ...
        expect(currentGame2.firstPlayerProgress.score).toBeDefined();
        expect(currentGame2.secondPlayerProgress.player.id).toEqual(user4.id);
        expect(currentGame2.secondPlayerProgress.player.login).toEqual(
          user4.login,
        );
        expect(currentGame2.secondPlayerProgress.answers.length).toBe(1);
        ...
        expect(currentGame3.id).toBeDefined();
        expect(currentGame3.status).toEqual(GameStatuses.Active);
        expect(currentGame3.firstPlayerProgress.player.id).toEqual(user3.id);
        expect(currentGame3.firstPlayerProgress.player.login).toEqual(
          user3.login,
        );
        expect(currentGame3.firstPlayerProgress.answers).toBeDefined();
        ...
        expect(currentGame3.firstPlayerProgress.score).toBeDefined();
        expect(currentGame3.secondPlayerProgress.player.id).toEqual(user4.id);
        expect(currentGame3.secondPlayerProgress.player.login).toEqual(
          user4.login,
        );
        expect(currentGame3.secondPlayerProgress.answers.length).toBe(2);
